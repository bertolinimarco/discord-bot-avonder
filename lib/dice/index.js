...
const parseDiceGroup = string => {
  ...
  if (rest === string) throw new ParsingError(`Non capisco cosa intendi con \`${rest}\``);

  while (operator !== false && rest !== false) {
    [operator, rest] = parser.takeNext(rest, maps.ruleString);
    if (!operator) throw new ParsingError(`Non so cosa sia \`${rest}\``);
  ...
  try {
    ...
    if (count > 30 && type !== rollTypes.MOD)
      throw new ParsingError(
        `Meglio di no, Discord esploderebbe... \`(${string})\``
      );
    if (size > 100)
      throw new ParsingError(
        `Purtroppo non ho dadi con più di 100 facce \`(${string})\``
      );
    ...
    throw new ParsingError(`Non capisco cosa intendi con \`${string}\``);
  }
};
...
const handleRoll = string => {
  ...
  try {
    ...
    if (sets.length > 10)
      throw new ParsingError(
        `Mmm... ${sets.length} tiri, davvero? Prova con 10 (massimo)`
      );
  } catch (e) {
    if (e.name === "ParsingError") throw new ParsingError(e.message);
    else throw new Error(`Questo comando non ha senso :scream:`);
  }
  return { string, sets };
};
...
const handler = params => {
  try {
    ...
    if (result.length > 2000)
      throw new ParsingError(
        `Ci sono troppi risultati... almeno Discord non è esploso :scream: \`(${params})\``
      );
    return result;
  } catch (e) {
    console.error(`Qualcosa non va: '${params}' (${e.message})`);
    return e.message;
  }
};
