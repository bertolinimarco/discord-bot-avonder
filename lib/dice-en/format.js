...
const formatGroup = group => {
  switch (group.type) {
    case rollTypes.MOD:
      return `${group.result}`;
    case rollTypes.WILD:
      return `${group.string} [ ${group.dice
        .map(formatPieceWithMarker)
        .join(", ")} ]`;
    case rollTypes.SUM:
      return `${group.string} [ ${mapExplosions(group)
        .map(formatPieceWithMarker)
        .join(", ")} ]`;
    case rollTypes.FATE:
      return `${group.dice.length} [ ${group.dice
        .map(formatPieceWithMarker)
        .join(", ")} ]`;
    case rollTypes.POOL:
      return `${group.dice[0].size} [ ${mapExplosions(group)
        .map(formatPieceWithMarker)
        .join(", ")} ]`;
  }
};
...
const formatSetGroups = set => {
  return set.groups.length
    ? `\n${formatGroups(set.groups)} ⟶ ${set.result}`
    : undefined;
};
...
const formatRoll = roll => {
  if (roll.string === false)
    return "What kind of roll? Try with `Avonder help dice`";
  try {
    ...
    throw new FormattingError("I think i messed up with the results...");
  }
};
