const helpCards = () => `You can draw cards with \`Avonder draw\` or \`!c\` for short. Valid commands are: \`\`\`
!c                        - draw 1 card
...
const helpDice = () => `You can roll dice with \`Avonder roll\` or \`!r\` for short.

Dice format is <?number><**type**><?number><?**modifier**><?number> where "number" is any whole number, and ? means optional.

- Ask me for more information about about dice **types** with \`Avonder help types\`.
- I support a bunch of **modifiers** for your rolls, like exploding, keep-the-lowest, keep-the-highest, and more. Ask me for more information with \`Avonder help modifiers\`.
...
-----------------------------------------------------------
Avonder help               (none)      Avonder help
Avonder roll               !r          Avonder help dice
Avonder draw               :d          Avonder help cards
Avonder private <command>  (none)      Avonder help private
\`\`\`
`;

const helpTypes = () => `Die types are \`d\`ie, \`p\`ool, \`w\`ild, and \`f\`ate.

Normal die rolls use \`d\`. This roll type rolls each die and adds up the result. I support rules **modifiers** for things like exploding dice and keep-the-lowest!keep-the-highest (see \`Avonder help modifiers\` for more info).
\`\`\`
...
const helpPrivate = () => `You can do commands in secret using \`Avonder secret <command>\`.
...
const handler = (params /*, author, guildId */) => {
  ...
  return `
  You can ask for help about: ${[...helpTopics.keys()]
    .map(topic => `\`${topic}\``)
    .join(", ")}
`;
};
