...
const parseDiceGroup = string => {
  ...
  if (rest === string)
    throw new ParsingError(`I'm not sure what you mean with \`${rest}\``);

  while (operator !== false && rest !== false) {
    [operator, rest] = parser.takeNext(rest, maps.ruleString);
    if (!operator)
      throw new ParsingError(`I don't know what \`${rest}\` is...`);
...
const handleGroup = (string, op = combinatorTypes.ADD) => {
  try {
    ...
    if (count > 30 && type !== rollTypes.MOD)
      throw new ParsingError(`Mmm, Discord would explode with \`(${string})\``);
    if (size > 100)
      throw new ParsingError(
        `Sadly, I don't have dices with more than 100 faces \`(${string})\``
      );

    if (ruleSet.length)
      ruleSet = ruleSet.map(([k, v]) => [maps.ruleOps.get(k), v]);
    ...
    throw new ParsingError(`I'm not sure what you mean with \`${string}\``);
  }
};
...
const handleRoll = string => {
  ...
  try {
    ...
    if (sets.length > 10)
      throw new ParsingError(
        `Mmm... ${sets.length} rolls, really? Try with 10 (Yes, max is 10)`
      );
  } catch (e) {
    if (e.name === "ParsingError") throw new ParsingError(e.message);
    else throw new Error(`This command have no meaning :scream:`);
  }
  return { string, sets };
};
...
const handler = params => {
  try {
    ...
    if (result.length > 2000)
      throw new ParsingError(
        `Too many results... at least Discord isn't exploded :scream: \`(${params})\``
      );
    return result;
  } catch (e) {
    console.error(`Something is wrong: '${params}' (${e.message})`);
    return e.message;
  }
};
